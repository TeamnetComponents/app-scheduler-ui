...
schedulerControllers
    .controller('ScheduleControllerOk', function ($scope,AppGridConstants, Schedule, TimeInterval, Month, DayOfWeek, RecurrentTimeUnit,  ScheduledJob) {
        ...
        $scope.repetitionTypes = [{id : "1", name:"Regular intervals"}, {id:"2", name:"Custom fire times"}];

        $scope.selectedIntervalType = 1;
        $scope.intervalTypes = [{id : "1", name:"Predefined intervals"}, {id:"2", name:"Custom defined intervals"}];
        ...
        $scope.changeRepetitionType = function() {
            console.log("Selected rep type : " + $scope.selectedRepetitionType);
            if ($scope.selectedRepetitionType == 1) {
                $scope.schedule.regularIntervals = true;
            }
            else if ($scope.selectedRepetitionType == 2) {
                $scope.schedule.regularIntervals = false;
            }

        }

        $scope.changeIntervalType = function() {
            if ($scope.selectedIntervalType == 1) {
                $scope.schedule.predefinedIntervals = true;
            }
            else if ($scope.selectedIntervalType == 2) {
                $scope.schedule.predefinedIntervals = false;
            }
        }
        ...
        $scope.clear = function () {
            $scope.schedule = {active: true, recurrent: false, predefinedIntervals: true, regularIntervals: true, cron: null, startTime: null, endTime: null, repetitions: null, id: null};
            $scope.searchTerms = [];
            $scope.showDetails=false;
        };
